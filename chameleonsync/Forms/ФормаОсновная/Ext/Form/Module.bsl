...
// Требует добавления численного реквизита chameleon_id в справочник номенклатуры, контрагентов и документ поступления товаров и услуг
// </Описание>
...
Процедура ВыполнитьНаСервере()
	...
	ИначеЕсли Объект.ТипЗагрузки = 8 Тогда
		ЗагрузитьКонтрагентов(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 9 Тогда
...
&НаСервере
Функция ЗагрузитьТаблицуКонтрагентов(PostgreSQLObject, Отказ)
	ТаблицаКонтрагентов = ВернутьТаблицуКонтрагентов();
	...
	| 	id_price_level
	| FROM front.clients
	| WHERE is_supplier = TRUE AND active = TRUE
	| ORDER BY id_client";
	...
	Пока РезультатЗапроса.EOF() = 0  Цикл
		...
		Для Каждого Колонка ИЗ ТаблицаКонтрагентов.Колонки Цикл 
			СтрокаТаблицыКонтрагентов[Колонка.Имя] = РезультатЗапроса.Fields(Колонка.Имя).Value;
		КонецЦикла;
...
&НаСервере
Процедура ЗагрузитьКонтрагентов(PostgreSQLObject, Отказ)
	ТаблицаКонтрагентов = ЗагрузитьТаблицуКонтрагентов(PostgreSQLObject, Отказ);
	ТаблицаКонтрагентовКолонки = ТаблицаКонтрагентов.Колонки;
	ТаблицаКонтрагентовКолонки.Добавить("Документ");
	ТаблицаКонтрагентовКолонки.Добавить("ПредставлениеДокумента");
	ТаблицаКонтрагентовКолонки.Добавить("Пометка");
	ТаблицаКонтрагентовКолонки.Добавить("ПредставлениеСтрокиХамелеона");
	...
	Для Каждого Контрагент Из ТаблицаКонтрагентов Цикл
		...
		|ГДЕ
		|	Контрагент.chameleon_id = &Код"; 
		Запрос.УстановитьПараметр("Код", Контрагент.id_client); 
		
		РезультатЗапроса = Запрос.Выполнить().Выгрузить(); 
		
		Контрагент.Пометка = Истина; 			
		Контрагент.ПредставлениеСтрокиХамелеона = " №: " + Контрагент.id_client;
		
		Если РезультатЗапроса.Количество() = 0 Тогда  /// Тут создать документ 
			Контрагент.Документ = Неопределено;
			Контрагент.ПредставлениеДокумента = "<Новый> - " + Контрагент.name_client;
		ИначеЕсли РезультатЗапроса.Количество() = 1 Тогда //// Тут получить по ссылке объект 
			Контрагент.Документ = РезультатЗапроса[0].Ссылка;
			Контрагент.ПредставлениеДокумента = Строка(РезультатЗапроса[0].Ссылка);
			Контрагент.Пометка = Ложь;
		Иначе
			Сообщить("Существует несколько документов номенклатуры с одним номером " + Контрагент.id_client); 
			Возврат;
		КонецЕсли;		
	КонецЦикла;
		
	//Сохраняям результат
	КопироватьРеквизитыТаблицыЗначений(ТаблицаКонтрагентов, Объект.ТаблицаДокументов.Выгрузить(), "Объект.ТаблицаДокументов"); 
	Объект.ТаблицаДокументов.Загрузить(ТаблицаКонтрагентов);
КонецПроцедуры

&НаСервере
Процедура СохранитьКонтрагентов()
	ТаблицаКонтрагентов = Объект.ТаблицаДокументов.Выгрузить();
	
	Для Каждого Контрагент Из ТаблицаКонтрагентов Цикл
				
		// Пропустим неотмеченные документы
		Если НЕ Контрагент.Пометка Тогда
			Продолжить;
		КонецЕсли;
		
		Код = Контрагент.id_client; 	
		Наименование = Контрагент.name_client;
		ВозможностьВозврата = Контрагент.can_return;
		УровеньЦен = Контрагент.id_price_level;
					
		Если Контрагент.Документ = Неопределено Тогда			
        	НовыйЭлемент = Справочники.Контрагенты.СоздатьЭлемент();
			
			//Может вызвать коллизии
			//НовыйЭлемент.Код = Код;
			
			//Индекс сквозной нумерации с базой хамелеона
			НовыйЭлемент.chameleon_id = Код;     
			НовыйЭлемент.Наименование = Наименование;
			НовыйЭлемент.НаименованиеПолное = Наименование;	
			НовыйЭлемент.Записать();
			
		Иначе
			ИзменяемыйЭлемент = Контрагент.Документ.ПолучитьОбъект();
			ИзменяемыйЭлемент.Наименование = Наименование;  			
			ИзменяемыйЭлемент.Записать();
			
		КонецЕсли;

		Сообщить("Записан контрагент: " + Код + " - " + Наименование);
	КонецЦикла;
...
Функция ЗагрузитьТаблицуЕдиницИзмерений(PostgreSQLObject, Отказ)
	ТаблицаЕдиницИзмерений = ВернутьТаблицуЕдиницИзмерений();
	...
	Пока РезультатЗапроса.EOF() = 0  Цикл
		...
		СтрокаТаблицаЕдиницИзмерений = ТаблицаЕдиницИзмерений.Добавить();
		Для Каждого Колонка ИЗ ТаблицаЕдиницИзмерений.Колонки Цикл 
			СтрокаТаблицаЕдиницИзмерений[Колонка.Имя] = РезультатЗапроса.Fields(Колонка.Имя).Value;
		КонецЦикла;
				
		РезультатЗапроса.MoveNext();    
	КонецЦикла;
	
	Возврат ТаблицаЕдиницИзмерений;
КонецФункции	

&НаСервере
Функция ЗагрузитьТаблицуНоменклатуры(PostgreSQLObject, Отказ)
	ТаблицаНоменклатуры = ВернутьТаблицуНоменклатуры();
	...
	Пока РезультатЗапроса.EOF() = 0  Цикл	
		...
		Для Каждого Колонка ИЗ ТаблицаНоменклатуры.Колонки Цикл 
			СтрокаТаблицаНоменклатуры[Колонка.Имя] = РезультатЗапроса.Fields(Колонка.Имя).Value;
		КонецЦикла;
...
Процедура ЗагрузитьНоменклатуру(PostgreSQLObject, Отказ)
	ТаблицаЕдиницИзмерений = ЗагрузитьТаблицуЕдиницИзмерений(PostgreSQLObject, Отказ);
	
	ТаблицаНоменклатуры = ЗагрузитьТаблицуНоменклатуры(PostgreSQLObject, Отказ);
	ТаблицаНоменклатурыКолонки = ТаблицаНоменклатуры.Колонки;
	ТаблицаНоменклатурыКолонки.Добавить("Документ");
	ТаблицаНоменклатурыКолонки.Добавить("ПредставлениеДокумента");
	ТаблицаНоменклатурыКолонки.Добавить("Пометка");
	ТаблицаНоменклатурыКолонки.Добавить("ПредставлениеСтрокиХамелеона");
	ТаблицаНоменклатурыКолонки.Добавить("ЕдиницыИзмерений");
	...
	Для Каждого Номенклатура Из ТаблицаНоменклатуры Цикл
		...
		|ГДЕ
		|	Номенклатура.chameleon_id = &Код"; 
		Запрос.УстановитьПараметр("Код", ""+Номенклатура.id_goods); 
		РезультатЗапроса = Запрос.Выполнить().Выгрузить(); 
		
		Номенклатура.Пометка = Истина; 			
		Номенклатура.ПредставлениеСтрокиХамелеона = "№: " + Номенклатура.id_goods;
		
		//Ищем все единицы измерений для данного товара
		Отбор = Новый Структура("id_goods", Номенклатура.id_goods);
		Номенклатура.ЕдиницыИзмерений = ТаблицаЕдиницИзмерений.Скопировать(Отбор);
		
		Если РезультатЗапроса.Количество() = 0 Тогда  /// Тут создать документ 
			Номенклатура.Документ = Неопределено;
			Номенклатура.ПредставлениеДокумента = "<Новый> - " + Номенклатура.name_goods;
		ИначеЕсли РезультатЗапроса.Количество() = 1 Тогда //// Тут получить по ссылке объект 
			Номенклатура.Документ = РезультатЗапроса[0].Ссылка;
			Номенклатура.ПредставлениеДокумента = Строка(РезультатЗапроса[0].Ссылка);
			Номенклатура.Пометка = Ложь;
		Иначе
			Сообщить("Существует несколько документов номенклатуры с одним номером " + Номенклатура.id_goods); 
			Возврат;
		КонецЕсли;		
	КонецЦикла;
		
	//Сохраняям результат
	КопироватьРеквизитыТаблицыЗначений(ТаблицаНоменклатуры, Объект.ТаблицаДокументов.Выгрузить(), "Объект.ТаблицаДокументов"); 
	Объект.ТаблицаДокументов.Загрузить(ТаблицаНоменклатуры);
КонецПроцедуры

&НаСервере
Процедура СохранитьНоменклатуру()
	ТаблицаНоменклатуры = Объект.ТаблицаДокументов.Выгрузить();
	
	Для Каждого Номенклатура Из ТаблицаНоменклатуры Цикл
				
		// Пропустим неотмеченные документы
		Если НЕ Номенклатура.Пометка Тогда
			Продолжить;
		КонецЕсли;
		
		Код = Номенклатура.id_goods;
		ВидНоменклатуры = ПолучитьВидНоменклатуры(Номенклатура.type_goods);
		НазваниеЕдиницыИзмерения = Номенклатура.name_unit;
		
		БазоваяЕдиницаИзмерения = Справочники.КлассификаторЕдиницИзмерения.НайтиПоНаименованию(НазваниеЕдиницыИзмерения);
		Если БазоваяЕдиницаИзмерения.Пустая() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("СохранитьНоменклатуру(): Нет Базовой Единицы Измерения " + НазваниеЕдиницыИзмерения);	
		КонецЕсли;
			
		Артикул = Номенклатура.bar_code; // Исправить Использовать штрихкоды
		Наименование = Номенклатура.name_goods;
		
		СтавкаНДС = Перечисления.СтавкиНДС.БезНДС; 
		Если Номенклатура.rate_tax = 0.00 Тогда
			СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
		ИначеЕсли Номенклатура.rate_tax = 7.00 Тогда
			СтавкаНДС = Перечисления.СтавкиНДС.НДС7;
		ИначеЕсли Номенклатура.rate_tax = 20.00 Тогда
			СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
		Иначе
			ОбщегоНазначения.СообщитьОбОшибке("СохранитьНоменклатуру(): Нет соответствия налоговой ставки");	
		КонецЕсли;
		
		Если Номенклатура.Документ = Неопределено Тогда
			НовыйЭлемент = Справочники.Номенклатура.СоздатьЭлемент();
			
			//Может создать коллизию
			НовыйЭлемент.Код = Код;	
			
			//Индекс для сквозной нумерации с базой хамелеона
			НовыйЭлемент.chameleon_id = Код;
			...
			// Единицы измерений
			Для Каждого Единица Из Номенклатура.ЕдиницыИзмерений Цикл
				ДополнительнаяЕдиницаИзмерения = ПолучитьЕдиницуИзмерения(НовыйЭлемент, Единица.name_unit, Единица.rate);
		...
		Иначе
			ИзменяемыйЭлемент = Номенклатура.Документ.ПолучитьОбъект(); 
			
			//Индекс для сквозной нумерации с базой хамелеона
			ИзменяемыйЭлемент.chameleon_id = Код;
			...
			// Единицы измерений
			Для Каждого Единица Из Номенклатура.ЕдиницыИзмерений Цикл
				ДополнительнаяЕдиницаИзмерения = ПолучитьЕдиницуИзмерения(ИзменяемыйЭлемент, Единица.name_unit, Единица.rate);
		...
		Сообщить("Записана номенклатура: " + Код + " - " + Наименование);
	КонецЦикла;
...
Процедура ЗагрузитьОтчетОРозничныхПродажах(PostgreSQLObject, Отказ)
	...
	Для Каждого Продажа Из ТаблицаПродаж Цикл
		...
		Для Каждого Чек Из Чеки Цикл
			...
			Для Каждого СтрокаЧека Из Чек.СтрокиЧека Цикл
				...
				Товар = Справочники.Номенклатура.НайтиПоРеквизиту("chameleon_id", СтрокаЧека.id_goods);
				...
				//УЛУЧШИТЬ: Исправить отбор единственной единицы по владельцу
	...
	Для Каждого ДанныеДляДокумента Из ТаблицаРозничныхПродаж Цикл	
		...
		Если ТЗ.Количество() = 0 Тогда  /// Тут создать документ 
			...
			ДанныеДляДокумента.ПредставлениеДокумента = "<Новый> - " + Формат(ДанныеДляДокумента.ДатаДокумента, "ДФ=дд.ММ.yyyy");
		ИначеЕсли ТЗ.Количество() = 1 Тогда //// Тут получить по ссылке объект 
			...
			Если ДанныеДляДокумента.Документ.Проведен Тогда
				ДанныеДляДокумента.ПредставлениеДокумента = "<Проведен> - " + ДанныеДляДокумента.ПредставлениеДокумента;
			КонецЕсли;
		Иначе
...
Процедура СохранитьОтчетРозничныхПродаж()
	...
	Для Каждого ДанныеДляДокумента Из ТаблицаРозничныхПродаж Цикл
		...
		Если ДанныеДляДокумента.Документ = Неопределено Тогда
			...
			// 333 счет
			Док.СчетУчетаРасчетовСЭквайрером = ПланыСчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте;
			
			Док.КассаККМ = ДанныеДляДокумента.ККМ;
			
			// Организация
			Организация = ДанныеДляДокумента.ККМ.Владелец;
		...
		Для Каждого СтрокаБезнала Из ДанныеДляДокумента.ТаблицаСтрокЧекаБезнал Цикл
			...
			// УЛУЧШИТЬ: Безнальный по-умолчанию 
		...
		КонецЕсли;
		Если РегламентированнаяОтчетность.ИДКонфигурации() = "УТП" Тогда
			ВТ.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры, Цена, ЕдиницаИзмерения, СтавкаНДС, Коэффициент, ПроцентСкидкиНаценки, СхемаРеализации, НалоговоеНазначение, СчетУчетаБУ", "Количество,Сумма");
		КонецЕсли;
...
Функция ЗагрузитьТаблицуПоступлений(PostgreSQLObject, Отказ)
	...
	СтрокаЗапроса = СтрШаблон(
		"SELECT *
		| FROM spring.doc_invoice_head
		| WHERE date_doc BETWEEN %1 AND %2 AND status != 0 %3", 
	...
	СтрокаЗапроса = СтрШаблон(
		"SELECT lines.* 
		| FROM spring.doc_invoice_table AS lines INNER JOIN spring.doc_invoice_head AS head ON lines.id_doc=head.id_doc 
		| WHERE head.date_doc BETWEEN %1 AND %2 AND head.status != 0 %3",
		СтрокаДатаЗагрузкиНачало, СтрокаДатаЗагрузкиКонец, СтрокаСклада);
...
Процедура ЗагрузитьОтчетПоступленияТоваровУслуг(PostgreSQLObject, Отказ)
	...
	Для Каждого ДокументыТочки Из ТаблицаПоступлений Цикл
		Для Каждого ДокументПоступления Из ДокументыТочки.ДанныеПоступлений Цикл
			...
			ДатаДокумента = ПреобразоватьВДату(ДокументПоступления.date_doc);
			...
			|	И ПоступлениеТоваровУслуг.chameleon_id = &НомерДокумента";
			
			Запрос.УстановитьПараметр("Склад", Склад ); 
			//Запрос.УстановитьПараметр("НачалоДняДата", НачалоДня(ДатаДокумента));
			//Запрос.УстановитьПараметр("КонецДняДата", КонецДня(ДатаДокумента));
			Запрос.УстановитьПараметр("НомерДокумента", ДокументПоступления.id_doc); 
			...
			СтрокаПоступления.ПредставлениеСтрокиХамелеона = ""+СтрокаПоступления.Склад + 
				" №: "+ДокументПоступления.number_doc + 
				" от " + ДатаДокумента;
			...
			Если РезультатЗапроса.Количество() = 0 Тогда  /// Тут создать документ 
				...
				СтрокаПоступления.ПредставлениеДокумента = "<Новый> - " + ДокументПоступления.number_doc + " от " + ДатаДокумента;
			ИначеЕсли РезультатЗапроса.Количество() = 1 Тогда //// Тут получить по ссылке объект 
				...
				Если СтрокаПоступления.Документ.Проведен Тогда
					СтрокаПоступления.ПредставлениеДокумента = "<Проведен> - " +СтрокаПоступления.ПредставлениеДокумента;
				КонецЕсли;
			Иначе  						
				Сообщить("Существует несколько документов ПоступлениеТоваровУслуг с одним номером " + ДокументПоступления.number_doc); 
				Возврат;
			КонецЕсли; 
...
Процедура СохранитьПоступлениеТоваровУслуг()
	...
	Для Каждого ДокументПоступления Из ТаблицаПоступлений Цикл
		...
		Если ДокументПоступления.Документ = Неопределено Тогда  /// Тут создать документ 
			...
			//Может вызвать коллизию
			//Док.Номер = ДокументПоступления.number_doc;
			//Индекс базы хамелеона
			Док.chameleon_id = ДокументПоступления.id_doc;
			...
			Док.Контрагент = Справочники.Контрагенты.НайтиПоРеквизиту("chameleon_id", ДокументПоступления.id_client);
			...
			Организация = Док.ДоговорКонтрагента.Организация;
			Если Организация.Пустая() Тогда
				Организация = Объект.Организация;
			КонецЕсли;
			Док.Организация = Организация;
			...
			//Счета
			//УЛУЧШИТЬ: Вынести в реквизиты - 631 счет?
			Док.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыСОтечественнымиПоставщиками;
			Док.СчетУчетаРасчетовПоАвансам = ПланыСчетов.Хозрасчетный.РасчетыСОтечественнымиПоставщиками;
		...
		Для Каждого СтрокаДокумента Из ДокументПоступления.СтрокиДокумента Цикл 	
			...
			Товар = Справочники.Номенклатура.НайтиПоРеквизиту("chameleon_id", СтрокаДокумента.id_goods);
			Если Товар.Пустая() Тогда
...
&НаСервере
Функция ПолучитьЕдиницуИзмерения(Номенклатура, НазваниеЕдиницы, Коэффициент)
	...
	Если ДополнительнаяЕдиницаИзмерения.Пустая() Тогда
	    ...
	    ДополнительнаяЕдиницаИзмерения.Коэффициент = Коэффициент;
	    Попытка
	        ДополнительнаяЕдиницаИзмерения.Записать();
	    Исключение
	        Сообщить("ПолучитьЕдиницуИзмерения() " + ОписаниеОшибки());
		КонецПопытки;
	КонецЕсли;
	
	Возврат ДополнительнаяЕдиницаИзмерения.Ссылка;
...
&НаСервере
Функция ВернутьТаблицуНоменклатуры()
	ТаблицаНоменклатуры = Новый ТаблицаЗначений;
	ТаблицаНоменклатурыКолонки = ТаблицаНоменклатуры.Колонки;
	ТаблицаНоменклатурыКолонки.Добавить("id_goods");
	ТаблицаНоменклатурыКолонки.Добавить("id_group");
	ТаблицаНоменклатурыКолонки.Добавить("rate_tax");
	ТаблицаНоменклатурыКолонки.Добавить("name_unit");
	ТаблицаНоменклатурыКолонки.Добавить("bar_code");
	ТаблицаНоменклатурыКолонки.Добавить("name_goods");
	ТаблицаНоменклатурыКолонки.Добавить("type_goods");
	Возврат ТаблицаНоменклатуры;
КонецФункции

&НаСервере
Функция ВернутьТаблицуКонтрагентов()
	ТаблицаКонтрагентов = Новый ТаблицаЗначений;
	ТаблицаКонтрагентовКолонки = ТаблицаКонтрагентов.Колонки;
	ТаблицаКонтрагентовКолонки.Добавить("id_client");
	ТаблицаКонтрагентовКолонки.Добавить("name_client");
	ТаблицаКонтрагентовКолонки.Добавить("is_supplier");
	ТаблицаКонтрагентовКолонки.Добавить("can_return");
	ТаблицаКонтрагентовКолонки.Добавить("is_buyer");
	ТаблицаКонтрагентовКолонки.Добавить("id_price_level");
	Возврат ТаблицаКонтрагентов;
КонецФункции

&НаСервере
Функция ВернутьТаблицуЕдиницИзмерений()
	ТаблицаЕдиницИзмерений = Новый ТаблицаЗначений;
	ТаблицаЕдиницИзмеренийКолонки = ТаблицаЕдиницИзмерений.Колонки;
	ТаблицаЕдиницИзмеренийКолонки.Добавить("id_unit");
	ТаблицаЕдиницИзмеренийКолонки.Добавить("id_goods");
	ТаблицаЕдиницИзмеренийКолонки.Добавить("name_unit");
	ТаблицаЕдиницИзмеренийКолонки.Добавить("type_unit");
	ТаблицаЕдиницИзмеренийКолонки.Добавить("rate");
	Возврат ТаблицаЕдиницИзмерений;
КонецФункции
...
Процедура ПриОткрытии()
	...
	Если Объект.Организация.Пустая() Тогда
		Сообщить("Не найдена ваша организация!");
	КонецЕсли;
