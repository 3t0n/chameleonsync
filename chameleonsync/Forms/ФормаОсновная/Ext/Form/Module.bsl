...
Процедура ВыполнитьНаСервере()
	...
	Если Объект.ТипЗагрузки = 1 Тогда
		Загрузить_ОтчетОРозничныхПродажах(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 2 Тогда	
		Загрузить_ПоступлениеТоваровУслуг(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 3 Тогда
		Загрузить_ВозвратТоваровПоставщику(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 4 Тогда
		Загрузить_ПеремещениеТоваров(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 5 Тогда
		Загрузить_ИнвентаризациюТоваровНаСкладе(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 6 Тогда
		Загрузить_КассовыеДокументы(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 7 Тогда
		Загрузить_Номенклатуру(PostgreSQLObject, Отказ);
	ИначеЕсли Объект.ТипЗагрузки = 8 Тогда
		Загрузить_Переоценку(PostgreSQLObject, Отказ);
	КонецЕсли;	
...
&НаСервере
Процедура Загрузить_Номенклатуру(PostgreSQLObject, Отказ)
	Сообщить("Загрузка номенклатуры");
	
	Загрузить_ТаблицуЕдиницИзмерений(PostgreSQLObject, Отказ);
	//Загрузить_ТаблицуГруппНоменклатуры(PostgreSQLObject, Отказ);
	Загрузить_ТаблицуНоменклатуры(PostgreSQLObject, Отказ);
КонецПроцедуры

&НаСервере
Процедура Загрузить_ОтчетОРозничныхПродажах(PostgreSQLObject, Отказ)
	Отказ = Ложь;

	ТаблицаПродаж = Загрузить_ТаблицуЧеков(PostgreSQLObject, Отказ);
	ТаблицаПереоценок = Загрузить_ТаблицуПереоценок(PostgreSQLObject, Отказ);
	...
	Для Каждого Продажа Из ТаблицаПродаж Цикл
		...
		Для Каждого Чек Из Чеки Цикл
			...
			Для Каждого СтрокаЧека Из Чек.СтрокиЧека Цикл
				...
				//СтрокаТаблицыДокументов = Объект.ТаблицаДокументов.Найти(ИндексПоиска, "ИндексПоиска");
	...
	//Для Каждого ДанныеДляДокумента Из Объект.ТаблицаДокументов Цикл
	...
	//Объект.ТаблицаДокументов.ЗаполнитьЗначения(Истина, "Пометка");
	...
	//ТабличноеПоле = ТаблицаРозничныхПродаж;
...
Процедура СохранитьОтчетРозничныхПродаж()
	...
	Для Каждого ДанныеДляДокумента Из ТаблицаРозничныхПродаж Цикл
		...
		Если ДанныеДляДокумента.Документ = Неопределено Тогда
			...
			//Док.Склад = ДанныеДляДокумента.ККМ.chm_Склад; 
...
&НаСервере
Процедура Загрузить_ПоступлениеТоваровУслуг(PostgreSQLObject, Отказ)
	Перем мВалютаРегламентированногоУчета;
	Перем ПараметрОснование;
	Перем ПараметрОбъектКопирования;
	
	ТаблицаПоступлений = Загрузить_ТаблицуПоступлений(PostgreSQLObject, Отказ);
	
	Объект.ТаблицаДокументов.Очистить();
	КолонкиТаблицыДокументов = Объект.ТаблицаДокументов.Колонки;
	КолонкиТаблицыДокументов.Очистить();
	
	Объект.ТаблицаДокументов = ВернутьТаблицуПоступлений();
	...
	Для Каждого ДокументыТочки Из ТаблицаПоступлений Цикл
		Для Каждого ДокументПоступления Из ДокументыТочки.ДанныеПоступлений Цикл
			...
			ИначеЕсли ТЗ.Количество() = 1 Тогда //// Тут получить по ссылке объект 
				СтрокаПоступления.Документ = ТЗ[0].Ссылка;
				СтрокаПоступления.ПредставлениеДокумента = Строка(ТЗ[0].Ссылка);
	...
	// Сформируем документы на основании таблицы ТаблицаДокументов
	Для Каждого ДокументПоступления Из Объект.ТаблицаДокументов Цикл
...
&НаСервере
Процедура Загрузить_ВозвратТоваровПоставщику(PostgreSQLObject, Отказ)
	Перем мВалютаРегламентированногоУчета;
	Перем ПараметрОснование;
	Перем ПараметрОбъектКопирования;

	ТаблицаВозвратов = Загрузить_ТаблицуВозвратов(PostgreSQLObject, Отказ);
...
&НаСервере
Процедура Загрузить_ПеремещениеТоваров(PostgreSQLObject, Отказ)
	Перем мВалютаРегламентированногоУчета;
	Перем ПараметрОснование;
	Перем ПараметрОбъектКопирования;
	
	ТаблицаПеремещений = Загрузить_ТаблицуПеремещений(PostgreSQLObject, Отказ);
...
&НаСервере
Процедура Загрузить_ИнвентаризациюТоваровНаСкладе(PostgreSQLObject, Отказ)
	Перем мВалютаУпрУчета;
	Перем мВалютаРегламентированногоУчета;
	Перем мКурсУпрУчета;
	Перем мКратностьУпрУчета;

	ТаблицаИнвентаризаций = Загрузить_ТаблицуИнвентаризаций(PostgreSQLObject, Отказ);
...
&НаСервере
Процедура Загрузить_КассовыеДокументы(PostgreSQLObject, Отказ)
	Перем мВалютаРегламентированногоУчета;
	Перем ПараметрОснование;
	Перем ПараметрОбъектКопирования;
	ТаблицаКассовых = Загрузить_ТаблицуКассовых(PostgreSQLObject, Отказ);
...
&НаСервере
Функция Загрузить_ТаблицуПеремещений(PostgreSQLObject, Отказ)
...
&НаСервере
Функция Загрузить_ТаблицуВозвратов(PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
	РезультатКолонки.Добавить("НомерТорговойТочки");
...
&НаСервере
Функция Загрузить_ТаблицуКассовых(PostgreSQLObject, Отказ)
...
&НаСервере
Процедура Загрузить_Переоценку(PostgreSQLObject, Отказ)
...
&НаСервере
Функция Загрузить_ТаблицуИнвентаризаций(PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
	РезультатКолонки.Добавить("НомерТорговойТочки");
...
&НаСервере
Функция Загрузить_ТаблицуПоступлений(PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
	РезультатКолонки.Добавить("НомерТорговойТочки");
	...
	СтрокаЗапроса = "SELECT * FROM spring.doc_invoice_head WHERE date_doc BETWEEN " + СтрокаДатаЗагрузкиНачало + " AND " + СтрокаДатаЗагрузкиКонец;
	Если НЕ Объект.Склад = Справочники.Склады.ПустаяСсылка() Тогда
		СтрокаЗапроса = СтрокаЗапроса + " AND id_outlet="+Формат(Число(Объект.Склад.Код),"ЧГ=0; ЧН=0");
	КонецЕсли;
	СтрокаЗапроса = СтрокаЗапроса + " AND status != 0";
	...
	Пока РезЗапроса.EOF()=0  Цикл
		...
		Для Каждого Колонка ИЗ ТаблицаПоступлений.Колонки Цикл 
			Если Колонка.Имя = "СтрокиДокумента" Тогда
				продолжить;
			КонецЕсли;
			СтрокаТаблицыПоступлений[Колонка.Имя] = РезЗапроса.Fields(Колонка.Имя).Value;
		КонецЦикла; 
	...
	СтрокаЗапроса = "SELECT lines.* FROM spring.doc_invoice_table AS lines INNER JOIN spring.doc_invoice_head AS head ON lines.id_doc=head.id_doc WHERE head.date_doc BETWEEN " + СтрокаДатаЗагрузкиНачало + " AND " + СтрокаДатаЗагрузкиКонец;
	Если НЕ Объект.Склад = Справочники.Склады.ПустаяСсылка() Тогда
		СтрокаЗапроса = СтрокаЗапроса + " AND head.id_outlet="+Формат(Число(Объект.Склад.Код),"ЧГ=0; ЧН=0");
	КонецЕсли;
	...
	РезЗапроса = PostgreSQLQuery(PostgreSQLObject, Отказ, СтрокаЗапроса);
	Пока РезЗапроса.EOF()=0  Цикл
		СтрокаТаблицыПоступлений = ТаблицаПоступлений.Найти(РезЗапроса.Fields("id_doc").Value,"id_doc");
...
&НаСервере
Функция ВернутьТаблицуСтрокиПоступлений()
...
&НаСервере
Функция Загрузить_ТаблицуЕдиницИзмерений(PostgreSQLObject, Отказ)
...
&НаСервере
Функция Загрузить_ТаблицуГруппНоменклатуры(PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
...
&НаСервере
Функция Загрузить_ТаблицуНоменклатуры(PostgreSQLObject, Отказ)
...
&НаСервере
Функция Загрузить_ТаблицуПереоценок (PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
...
&НаСервере
Функция Загрузить_ТаблицуЧеков(PostgreSQLObject, Отказ)
	Результат = Новый ТаблицаЗначений;
	РезультатКолонки = Результат.Колонки;
	РезультатКолонки.Добавить("НомерРабочегоМеста");
	РезультатКолонки.Добавить("ДанныеЧеков");
	Сообщить("Загрузить_ТаблицуЧеков");
	...
	Пока РезЗапроса.EOF()=0  Цикл
		...
		Для Каждого Колонка ИЗ ТаблицаЧеков.Колонки Цикл 
			Если Колонка.Имя = "СтрокиЧека" Тогда
				продолжить;
			КонецЕсли; 
			СтрокаТаблицыЧеков[Колонка.Имя] = РезЗапроса.Fields(Колонка.Имя).Value; 
		КонецЦикла; 
	...
	РезЗапроса = PostgreSQLQuery(PostgreSQLObject, Отказ, СтрокаЗапроса);
	Пока РезЗапроса.EOF()=0  Цикл
		СтрокаТаблицыЧеков = ТаблицаЧеков.Найти(РезЗапроса.Fields("id_check").Value,"id_check");
		Если СтрокаТаблицыЧеков = Неопределено Тогда
			Отказ = Истина;
			Сообщить("Ошибка поиска чека по номеру: "+ РезЗапроса.Fields("id_check").Value);
			Прервать;
		КонецЕсли;
		...
		Для Каждого Колонка ИЗ СтрокиЧека.Колонки Цикл 
			СтрокаСтрокиЧеков[Колонка.Имя] = РезЗапроса.Fields(Колонка.Имя).Value; 
		КонецЦикла;
...
Процедура КопироватьРеквизитыТаблицыЗначений(ТаблицаЗначенийОткуда, ТаблицаЗначенийКуда, ПутьКуда)
	...
	Для Каждого Колонка Из ТаблицаЗначенийКуда.Колонки Цикл
		Если Колонка.Имя = "Пометка" 
			...
			Или Колонка.Имя = "ДокументВБазе" Тогда
